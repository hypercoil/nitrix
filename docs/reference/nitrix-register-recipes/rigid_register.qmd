---
title: "rigid_register"
generated: true
description: "Estimate the rigid transform aligning `moving` to `fixed`."
---

[‹ back to `nitrix.register.recipes`](../nitrix-register-recipes.qmd)

<!-- nitrix:gen sig:rigid_register -->
```python
rigid_register(moving: Float[Array, '*mspatial'], fixed: Float[Array, '*fspatial'], *, spec: RegistrationSpec = RegistrationSpec(), space: CoordinateSpace = IndexSpace(), method: str = 'auto', init: Literal['identity', 'moment'] = 'identity', init_transform: Optional[Float[Array, ' d1 d1']] = None, mask: Optional[Float[Array, '*fspatial']] = None, winsorize: Optional[tuple[float, float]] = None, histogram_match: bool = False) -> RegistrationResult
```
<!-- nitrix:/gen -->

<!-- nitrix:gen overview:rigid_register -->
Estimate the rigid transform aligning `moving` to `fixed`.

Optimises the 6-DOF (3-D) / 3-DOF (2-D) rigid Lie parameters
([`rigid_exp`](../nitrix-geometry.qmd)) coarse-to-fine so that `moving`
resampled by the result matches `fixed` under `spec.metric`.
<!-- nitrix:/gen -->

<!-- nitrix:gen params:rigid_register -->
**Parameters**

| parameter | type | shape | default | description |
|------------|--------|--------|----------|--------------------------------------------------------------|
| `moving` | `Float` | `*mspatial` | — | Single-channel images (2-D or 3-D). Shapes need not match (the warp is built on the `fixed` grid); the default [`IndexSpace`](../nitrix-register/IndexSpace.qmd) additionally assumes a shared voxel grid. |
| `fixed` | `Float` | `*fspatial` | — | Single-channel images (2-D or 3-D). Shapes need not match (the warp is built on the `fixed` grid); the default [`IndexSpace`](../nitrix-register/IndexSpace.qmd) additionally assumes a shared voxel grid. |
| `spec` | `RegistrationSpec` | — | `RegistrationSpec()` | The [`RegistrationSpec`](../nitrix-register/RegistrationSpec.qmd) (pyramid depth, iterations, metric, ...). **Metric choice:** [`SSD`](../nitrix-register/SSD.qmd) (default) within-modality -- the fast inverse-compositional path; [`MI`](../nitrix-register/MI.qmd) / [`CorrelationRatio`](../nitrix-register/CorrelationRatio.qmd) cross-modal; [`LNCC`](../nitrix-register/LNCC.qmd) under a smooth intensity bias. **Grid:** the default [`IndexSpace`](../nitrix-register/IndexSpace.qmd) assumes `moving` and `fixed` share a (roughly isotropic) voxel grid -- use [`WorldSpace`](../nitrix-register/WorldSpace.qmd) for anisotropic voxels or differing grids (a rigid in index space shears in physical space). |
| `space` | `CoordinateSpace` | — | `IndexSpace()` | Coordinate space to optimise in: [`IndexSpace`](../nitrix-register/IndexSpace.qmd) (default; voxel-space, shared-grid, on-device) or `WorldSpace(fixed_affine=..., moving_affine=...)` (physical space -- correct under anisotropic voxels and different grids). |
| `method` | `str` | — | `'auto'` | Solver: `"auto"` (default; the inverse-compositional fast path -- roughly 4-7x the forward throughput -- when its preconditions hold: [`IndexSpace`](../nitrix-register/IndexSpace.qmd) and a least-squares / SSD metric; the forward Gauss-Newton / LM path otherwise), `"inverse_compositional"` (force it; validates), or `"forward"`. The forward path is the parity oracle the fast path is tested against. |
| `init` | `Literal['identity', 'moment']` | — | `'identity'` | Starting transform: `"identity"` (default) or `"moment"` -- a centre-of-mass start (intensity-weighted centroids, plus a per-axis diagonal scale for affine) that lands inside the optimiser's basin on a large misalignment a single zero start would miss. [`IndexSpace`](../nitrix-register/IndexSpace.qmd) only. |
| `init_transform` | `Optional[Float[Array, ' d1 d1']]` | — | `None` | A self-contained warm-start matrix (a prior recipe's `result.matrix`), taking precedence over `init` -- the staged-pipeline hook (e.g. affine warm-started from rigid; see [`syn_pipeline`](../nitrix-register-recipes/syn_pipeline.qmd)). [`IndexSpace`](../nitrix-register/IndexSpace.qmd) only. |
| `mask` | `Optional[Float[Array, '*fspatial']]` | — | `None` | Optional non-negative per-voxel weight on the `fixed` grid: the cost is restricted to / weighted by it (a brain mask, lesion exclusion, FoV), with out-of-mask voxels ignored -- for [`MI`](../nitrix-register/MI.qmd) / [`CorrelationRatio`](../nitrix-register/CorrelationRatio.qmd) it gates the joint-histogram scatter, not just the reduction. A mask routes to the **forward** path (the inverse-compositional constant-template Hessian assumes the full grid); `method='inverse_compositional'` with a mask raises. |
| `winsorize` | `Optional[tuple[float, float]]` | — | `None` | Intensity conditioning before registration (the fMRIPrep front-end): `winsorize=(0.005, 0.995)` clips each image to those percentiles (outlier-robust); `histogram_match=True` remaps `moving` onto `fixed`'s distribution (within-modality). Both default off (byte-unchanged). They condition the *estimate*, so `warped` is the conditioned moving -- apply `matrix` to the original for the original intensities. |
| `histogram_match` | `bool` | — | `False` | Intensity conditioning before registration (the fMRIPrep front-end): `winsorize=(0.005, 0.995)` clips each image to those percentiles (outlier-robust); `histogram_match=True` remaps `moving` onto `fixed`'s distribution (within-modality). Both default off (byte-unchanged). They condition the *estimate*, so `warped` is the conditioned moving -- apply `matrix` to the original for the original intensities. |

**Returns** `RegistrationResult`. A [`RegistrationResult`](../nitrix-register/RegistrationResult.qmd) carrying `matrix`, `params`, `warped`, and `cost_history`. `matrix` maps `fixed` to `moving` (index coordinates in [`IndexSpace`](../nitrix-register/IndexSpace.qmd), world coordinates in [`WorldSpace`](../nitrix-register/WorldSpace.qmd)); `warped` is `moving` resampled on the `fixed` grid.
<!-- nitrix:/gen -->

<!-- nitrix:gen perf:rigid_register -->
:::: {.callout-note title="Benchmarks — nitrix-perf-bench" collapse="true"}
GPU steady-state **29.27 ms** (median, L4).  [full report](https://github.com/hypercoil/nitrix-perf-bench/blob/main/reports/PERF_RIGID_REGISTER.md)
...
<!-- nitrix:gen dims:rigid_register -->
**Tensor dimensions**
