...
| `method` | `Interpolator` | — | `Linear()` | Interpolation kernel -- an [`Interpolator`](../nitrix-geometry-grid/Interpolator.qmd) record. Default [`Linear`](../nitrix-geometry-grid/Linear.qmd) ((multi-)linear, the prior behaviour). [`NearestNeighbour`](../nitrix-geometry-grid/NearestNeighbour.qmd) preserves label values exactly (the choice for integer segmentations when anti-aliasing is *not* wanted). |
