...
generated: true
description: "Per-point Jacobian of the deformation $ varphi =  mathrm{id} + u$."
---
...
Per-point Jacobian of the deformation $\varphi = \mathrm{id} + u$.

Here $\varphi$ is the deformation and `u` the displacement.
For a channel-last displacement field `u` with `d` spatial
axes, returns
$J[\dots, i, j] = \delta_{i,j} + \partial u_i / \partial x_j$,
...
<!-- nitrix:gen sections:jacobian_displacement -->
::: {.callout-note}
For folding detection use [`jacobian_det_displacement`](../nitrix-geometry-grid/jacobian_det_displacement.qmd) which
