...
samples, so -- unlike `CubicBSpline` -- it needs **no prefilter**.
...
Distinct from the other cubics: `CubicBSpline` is *approximating*
...
`Lanczos` is a windowed sinc.  Catmull-Rom is the cardinal cubic of
...
`Linear` (non-negative weights), whose output is bounded by its
neighbours.  This overshoot is the price of the higher-order fit; where a
strictly bounded, non-ringing resample is required, prefer `Linear`.
:::
<!-- nitrix:/gen -->
