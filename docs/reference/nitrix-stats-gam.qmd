...
`gam_fit` fits, per element (voxel / vertex / fixel), a generalised
additive model
...
`GAMResult` carries per-element coefficients, selected $\lambda$,
...
tests).  Partial effects are rendered with `smooth_partial_effect`.
