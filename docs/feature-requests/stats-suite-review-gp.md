...
### Round 3 — medium correctness/robustness hardening (✅ complete bar deferred PF4)
> **Done (11):** **ER3·ER5** `d260bb4` (GLM effective-rank dof + jit-traceable cluster
...
> across all LME tiers — matches statsmodels `llf`) · **DS3** `30bd4e0` (neutral
> `stats/_hsgp.py` — HSGP eigenstructure shared by gp/hgp/basis, byte-identical) · **DS2**
> (gp `_hsgp_design_pen`/`_exact_design_pen` closures now drive the hsgp+exact bodies too,
> byte-identical).
> **Deferred:** **PF4** (`Family`/`Link` value-hash — niche perf; needs param-encoded names,
> ripples to `'negbinomial'`-name tests).
...
**PF4** (`Family`/`Link` hashing — deferred), **DS2** ✅ (fit-driver), **DS3** ✅
(`_hsgp.py` de-dup).
