...
|---|---|---|---|---|
| **Functional alignment** (Procrustes / ProMises) | `atlas/promises.py` | `register` | [`register-functional-alignment`](resolved/register-functional-alignment.md) | the "empty" double-whitening is a self-flagged hack (PCA gauge ambiguity); cross-product orientation unverified |
| **Orthogonal Procrustes + subspace angles + image basis** | `atlas/totalangle.py`, `atlas/promises.py` | `linalg` | [`linalg-orthogonal-procrustes`](resolved/linalg-orthogonal-procrustes.md) | `arcsin/arccos` boundary sub-derivatives; reflection (`det<0`) handling |
...
| **Whitening** (ZCA + partial sphering) ✅ SHIPPED (`stats.whiten*` + `WhiteningState`; Newton–Schulz inverse-sqrt driver on `symsqrt`; PCA stays nimox) | `atlas/vmf.py::generalised_whitening` | `stats` (thin wrapper) | [`stats-whitening`](resolved/stats-whitening.md) | **estimator already existed** (`nimox.estimators.whitening` over `linalg.sympower`); shipped (A) findability wrapper + (B) cuSOLVER-free Newton–Schulz inverse-sqrt (certified ≈ `sympower(cov,−0.5)`); `cummax` repair rejected. nimox re-point filed. |
| **DCBC / DCCC parcellation-quality metric** ⏸ DEFERRED (dedicated parcellation sprint) | `atlas/dccc.py` | `metrics` | [`metrics-dccc`](metrics-dccc.md) | soft (DCCC) marginal-coaffiliation extension is **mathematically flawed** — conflates representation sharpness with quality, favours determinism (empirically confirmed 8/8 seeds, FR §0); relaxing DCBC to probabilistic is nontrivial/possibly ill-posed. Separate from the distance-binning kernel. Sprint-scoped |
| **Synthetic connectivity generators** ✅ SHIPPED (`augment.simulate`: band_limited_signals / **color_signals** [colouring = whitening inverse] / sparse_mixture_matrix + mix_signals / lowrank_block_connectome / markov_state_sequence) | `synthetic/scripts/{mix,sylo,corr,filter,denoise}.py` | `augment` | [`augment-synthetic-connectivity`](resolved/augment-synthetic-connectivity.md) | resolved: smooth spectral window (no brick-wall leakage); Bernoulli-mask + `lax.scan` (no data-dependent shapes); each ships a planted-structure recovery test |
...
- Atomised FRs: [`register-functional-alignment`](resolved/register-functional-alignment.md),
  [`linalg-orthogonal-procrustes`](resolved/linalg-orthogonal-procrustes.md),
  [`stats-directional-vmf`](stats-directional-vmf.md),
  [`stats-whitening`](resolved/stats-whitening.md), [`metrics-dccc`](metrics-dccc.md),
  [`augment-synthetic-connectivity`](resolved/augment-synthetic-connectivity.md).
