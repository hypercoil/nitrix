...
| **Whitening** (ZCA + partial sphering) ✅ SHIPPED (`stats.whiten*` + `WhiteningState`; Newton–Schulz inverse-sqrt driver on `symsqrt`; PCA stays nimox) | `atlas/vmf.py::generalised_whitening` | `stats` (thin wrapper) | [`stats-whitening`](stats-whitening.md) | **estimator already existed** (`nimox.estimators.whitening` over `linalg.sympower`); shipped (A) findability wrapper + (B) cuSOLVER-free Newton–Schulz inverse-sqrt (certified ≈ `sympower(cov,−0.5)`); `cummax` repair rejected. nimox re-point filed. |
