...
def test_forward_of_pure_harmonic_is_unit_coefficient():
    ...
    for ell, m in [(3, 2), (6, -4), (0, 0), (7, 7)]:
        ...
        off[ell, m + L] = 0.0
        np.testing.assert_allclose(abs(complex(c[ell, m + L])), 1.0, atol=1e-10)
...
def test_field_round_trip_is_band_limited_identity():
    ...
    np.testing.assert_allclose(np.asarray(recon), np.asarray(field), atol=1e-10)


def test_coefficient_triangular_mask():
    L = 6
    c = sht_forward(sht_inverse(_random_bandlimited_coeffs(L, 2)), band_limit=L)
    c = np.asarray(c)
    for ell in range(L + 1):
...
def test_batching_over_leading_dims():
    ...
    fields = jnp.stack([sht_inverse(c), sht_inverse(2.0 * c)])  # (2, n_lat, n_lon)
