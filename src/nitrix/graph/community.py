...
def _relaxed_modularity_sparse(
    ...
    if isinstance(A, ELL):
        AC = semiring_ell_matmul(
            A.values, A.indices, C,
            semiring=REAL, n_cols=A.n_cols, backend='jax',
        )
    else:  # SectionedELL
        AC = sectioned_semiring_ell_matmul(
            A, C, semiring=REAL, backend='jax',
        )
        ...
        if C_other is not C:
            if isinstance(A, ELL):
                AC_o = semiring_ell_matmul(
                    A.values, A.indices, C_other,
                    semiring=REAL, n_cols=A.n_cols, backend='jax',
                )
            else:
                AC_o = sectioned_semiring_ell_matmul(
                    A, C_other, semiring=REAL, backend='jax',
                )
        else:
            AC_o = AC
