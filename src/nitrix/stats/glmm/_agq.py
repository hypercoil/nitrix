...
The accuracy tier above Laplace: the random-slope marginal integral by an
``n_quad ** r`` tensor grid centred / scaled at each group's mode (which it
borrows from :mod:`._laplace`).  Split from the ``glmm`` monolith (audit O1).
"""
