...
@register_result(
    ...
    aux=('kernel', 'n_obs', 'rank', 'n_fixed', 'lo', 'hi', 'boundary'),
)
@dataclass(frozen=True)
class GPResult:
    """Per-element Gaussian-process fit output (HSGP engine).
    ...
    coef
        ...
        followed by the ``rank`` eigenfunction coefficients ``gamma``.
    cov_unscaled
    ...
    rank
        Reduced rank ``m`` (number of retained eigenfunctions).
    n_fixed
        Number of unpenalised fixed-effect columns ``M_0`` (intercept + parametric).
    lo, hi, boundary
        Domain descriptors recorded for re-evaluation: the data range ``[lo, hi]``
        and the boundary factor ``L = boundary * (hi - lo) / 2``.
    """
...
def gp_fit(
    ...
    *,
    ...
    lengthscale (HSGP engine; Gaussian responses).
    ...
    kernel
        ...
        ``'matern12'`` / ``'rbf'``.
    rank
        Reduced rank ``m`` (number of eigenfunctions).  Small ``rho`` needs a
        larger ``rank``; the default ``20`` covers the usual smooth range.
    boundary
        Domain-extension factor ``L / half-range`` (``>= 1``; default ``1.5``).
    bounds
        ``(lo, hi)`` data-range override (defaults to the data min/max).
    engine
        Reduced-rank engine.  ``'hsgp'`` (the only PR2 option).
    select
        Lengthscale-selection mode.  ``'shared-rho'`` (the only PR2 option): one
        ``rho`` across all elements, per-element amplitude and noise.
    rho_bounds
    ...
    corr
        Reserved for correlated-residual composition (PR3); must be ``None``.
    n_outer, n_search
    ...
    if engine != 'hsgp':
        raise NotImplementedError(
            f"gp_fit: engine={engine!r} -- only 'hsgp' is implemented (the "
            "full-rank 'exact' engine is PR3)."
        )
    ...
    if corr is not None:
        raise NotImplementedError(
            'gp_fit: corr= (correlated-residual composition) is PR3; pass None.'
        )
    if rank < 1:
    ...
    sqrt_lambda, phase, inv_sqrt_L = _hsgp_eigen(rank, c_mid, big_l, Y.dtype)
    ...
    X = jnp.concatenate(fixed_blocks + [phi_design], axis=1)  # (N, p)
    p = X.shape[1]
    m = rank
    ...
    # --- rho search: pooled REML on a fixed log-spaced grid -----------------
    span = max(hi - lo, 1e-6)
    if rho_bounds is None:
        rho_lo, rho_hi = 0.05 * span, 2.0 * span
    else:
        rho_lo, rho_hi = float(rho_bounds[0]), float(rho_bounds[1])
    log_rho_grid = jnp.asarray(
        np.linspace(np.log(rho_lo), np.log(rho_hi), int(n_rho)), dtype=Y.dtype
    )
...
def gp_predict(
    ...
    *,
    ...
    Reconstructs the fixed eigenfunction design at ``x_new`` (the eigenbasis is
    ``rho``-independent, so only the recorded domain ``(lo, hi, boundary)`` and
    ``rank`` are needed) and evaluates ``mean = X_new beta`` and the predictive
    variance ``sigma_e^2 diag(X_new V X_new^T)``.
