...
- ``winsorize`` -- clip each image to its low/high intensity percentiles
  ...
  ``_svf._normalise_step`` global-outlier sensitivity (the clipped image has no
  extreme voxel to set the trust-region cap).
- ``match_histogram`` -- remap the moving image's intensity distribution onto the
  reference's by CDF / quantile transport (``--use-histogram-matching 1``), so a
  within-modality SSD / CC metric is not fighting a global intensity gain/offset.
...
def winsorize(
    ...
    *,
    ...
    lo, hi = jnp.quantile(image, jnp.asarray([lower, upper], dtype=image.dtype))
...
def match_histogram(
    ...
    *,
    ...
    """Match ``moving``'s intensity histogram to ``reference``'s (quantile
    transport).
    ...
    r_q = jnp.quantile(reference.reshape(-1), qs)  # reference's, same quantiles
